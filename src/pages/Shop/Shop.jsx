...
import {
  ...
  ArrowUpRight, Smartphone, Apple, ChevronLeft, ChevronRight
} from 'lucide-react';
...
/* --- data --- */
const FEATURES = [
  { icon: Wind, title: 'True HEPA H13', desc: 'Captures 99.97% of particles down to 0.1 um - pet dander, smoke, mold, bacteria.' },
  ...
  { icon: Volume2, title: '360 deg Airflow', desc: 'Cylindrical design pulls air from every direction - no "dead zones" in the room.' },
];
...
const TESTIMONIALS = [
  ...
  },
  {
    ...
    rating: 5,
    text: '"Finally a purifier that doesn\'t look like an eyesore. Blends perfectly with any aesthetic and the 360 deg airflow actually works unlike cheaper cylindrical ones."',
  },
];
...
const FAQS = [
  ...
  { q: 'Does it remove viruses and bacteria?', a: 'The H13 True HEPA layer captures 99.97% of particles >= 0.1 um, which includes most airborne bacteria and viruses.' },
...
const Shop = () => {
  ...
  return (<div className="bg-[#FBFAF7] text-[#12281A] antialiased font-['Inter'] overflow-hidden">
            ...
            <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#E4EFDA] via-[#EBF4E3] to-[#D4E8C8] aspect-square flex items-center justify-center shadow-2xl shadow-black/10">
              {PRODUCT_SLIDES.map((slide, index) => (
                <img
                  ...
                  src={slide.src}
                  alt={slide.alt}
                  className={`absolute inset-0 h-full w-full object-contain p-6 drop-shadow-2xl transition-all duration-700 ease-out ${index === activeSlide ? 'opacity-100 scale-100' : 'pointer-events-none opacity-0 scale-95'}`}
                />
              ))}
              <button onClick={() => setActiveSlide(current => (current - 1 + PRODUCT_SLIDES.length) % PRODUCT_SLIDES.length)} aria-label="Previous product image" className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full bg-white/85 p-2 text-[#1E4D33] shadow-md transition hover:bg-white"><ChevronLeft size={20} /></button>
              <button onClick={() => setActiveSlide(current => (current + 1) % PRODUCT_SLIDES.length)} aria-label="Next product image" className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-white/85 p-2 text-[#1E4D33] shadow-md transition hover:bg-white"><ChevronRight size={20} /></button>
              <div className="absolute bottom-5 left-1/2 flex -translate-x-1/2 gap-2 rounded-full bg-white/80 px-3 py-2 backdrop-blur-sm">
                {PRODUCT_SLIDES.map((slide, index) => <button key={slide.src} onClick={() => setActiveSlide(index)} aria-label={`Show image ${index + 1}`} className={`h-2 rounded-full transition-all ${index === activeSlide ? 'w-6 bg-[#1E4D33]' : 'w-2 bg-[#1E4D33]/30'}`} />)}
              </div>
          ...
          <div data-reveal="">
            ...
            <div className="mt-6 mb-8 pb-8 border-b border-black/10">
              <div className="flex items-baseline gap-3">
                <span className="font-['Geist'] text-4xl font-semibold text-[#12281A]">Rs 4,999</span>
                <span className="text-lg text-black/30 line-through">Rs 18,999</span>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <button className="flex-1 flex items-center justify-center gap-2.5 bg-[#1E4D33] hover:bg-[#2A6647] active:scale-95 text-white font-medium text-sm px-7 py-3.5 rounded-full transition-all duration-300 shadow-xl shadow-[#1E4D33]/30 hover:shadow-2xl hover:shadow-[#1E4D33]/40">
                ...
                Add to Cart - Rs {(4999 * qty).toLocaleString()}
              </button>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-4" data-reveal="">
            ...
            {[
              { label: 'CADR (m3/h)', ours: 92, theirs: 61 },
    ...
    {/* APP PROMO */}
    <section className="bg-[#12281A] py-16 sm:py-20 overflow-hidden relative">
      ...
      <div className="relative max-w-7xl mx-auto px-6 sm:px-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        <div data-reveal="">
          <p className="text-xs uppercase tracking-[0.25em] text-[#42A85D] font-medium mb-4">EcoBreathe App</p>
          <h2 className="font-['Geist'] text-3xl sm:text-5xl font-light tracking-tighter text-white mb-5">
        ...
        <div className="flex justify-center lg:justify-end" data-reveal="">
          <div className="relative w-full max-w-[280px] sm:max-w-[320px] aspect-[9/16] rounded-[1.3rem]">
            <div className="absolute -inset-6 rounded-[2.5rem] bg-[#42A85D]/15 blur-3xl" />
            <div className="relative h-full overflow-hidden rounded-[2rem] border border-white/15 bg-white/5 p-3 shadow-2xl shadow-black/40 sm:p-4">
              <img src="/app.png" alt="EcoBreathe mobile app dashboard" className="h-full w-full rounded-[1.1rem] object-contain" />
            </div>
          </div>
        </div>
      </div>
