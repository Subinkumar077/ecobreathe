...
const Navbar = () => {
  ...
  const navClass = (path) => `px-3 py-1.5 rounded-full hover:text-black hover:bg-black/5 hover:no-underline transition-colors duration-300 ${location.pathname === path ? 'text-[#1E4D33] font-medium bg-[#E4EFDA]' : ''}`;
    ...
    <nav className="fixed left-1/2 -translate-x-1/2 top-4 sm:top-7 z-50 w-full max-w-fit px-4 pointer-events-none">
      <div className="flex items-center gap-1 bg-white/90 backdrop-blur-md rounded-full pl-4 pr-1.5 py-1.5 border border-black/10 shadow-xl shadow-black/10 pointer-events-auto">
        ...
        <div className="hidden md:flex items-center gap-1 text-sm text-black/60">
          ...
          <Link to="/rankings" className={navClass('/rankings')}>Rankings</Link>
          <Link to="/shop" className={navClass('/shop')}>Shop</Link>
        </div>
        <div className="ml-1 sm:ml-2 flex items-center"><div className="hidden sm:block w-48 mr-2"><SearchBar placeholder="Search city..." size="sm" /></div>{user ? <button onClick={handleLogout} className="bg-black/5 hover:bg-black/10 text-black text-sm font-medium px-4 sm:px-5 py-2 rounded-full transition-all duration-300 whitespace-nowrap">Log out</button> : <Link to="/auth" className="bg-[#1E4D33] hover:bg-[#2A6647] active:scale-95 text-white text-sm font-medium px-4 sm:px-5 py-2 rounded-full transition-all duration-300 whitespace-nowrap shadow-lg shadow-[#1E4D33]/30 hover:no-underline">Sign Up free</Link>}</div>
      </div>
    </nav>
  );
};
